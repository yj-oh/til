...
@AssertTrue | | true여야 함
@DecimalMax | value = String | 지정된 값보다 작거나 같은 실수여야 함
@DecimalMin | value = String | 지정된 값보다 크거나 같은 실수여야 함
@Future | | 날짜가 미래여야 함
@Past | | 날짜가 과거여야 함
@Max | value = long | 지정된 값보다 작거나 같아야 함
@Min | value = long | 지정된 값보다 크거나 같아야 함
@NotNull | | null이 아니어야 함
@Null | | null이어야 함
@Size | min = int, max = int | 
@Pattern | regexp = String | 정규식을 만족해야 함
